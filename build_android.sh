...
BUILD_TOOLS=android/sdk/build-tools/30.0.3
...
INCLUDES="-I. -Iinclude -I../include -I$NATIVE_APP_GLUE -I$TOOLCHAIN/sysroot/usr/include -Iraylib/src"
OBJECT_BUILD_DIR="android/build/obj"
...
for ABI in $ABIS; do
	...
	# Compile project
    # cmake -BDCMAKE_TOOLCHAIN_FILE=./android/ndk/build/cmake/android.toolchain.cmake -B build -S .
    # cmake --build build
	for file in src/*.c; do
		$CC -c $file -o "$OBJECT_BUILD_DIR/`basename $file`.o" \
			$INCLUDES -I$TOOLCHAIN/sysroot/usr/include/$CCTYPE $FLAGS $ABI_FLAGS
	done

        # Link the project with toolchain specific linker to avoid relocations issue.
	$TOOLCHAIN/bin/ld.lld $OBJECT_BUILD_DIR/*.o -o android/build/lib/$ABI/libmain.so -shared \
...
$BUILD_TOOLS/aapt package -f -m \
	...
	-I android/sdk/platforms/android-34/android.jar
